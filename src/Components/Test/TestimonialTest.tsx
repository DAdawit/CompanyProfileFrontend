  ...
  <p className={classNames("flex flex-wrap gap-0.5", className)} {...rest}>
    <span>
      {[...Array(5)].map((_, i) => {
        ...
        if (index <= Math.floor(rating)) content = <StarBorderPurple500Icon />;
        else if (rating > i && rating < index + 1)
          content = <StarBorderPurple500Icon />;
        else if (index > rating) content = <StarBorderPurple500Icon />;
    ...
    <div className="bg-white dark:bg-slate-800 shadow-xl rounded-xl hover:-translate-y-1 h-full duration-300 p-6">
      <div className="mt-4">
        <div className="flex justify-between items-center mb-6">
          ...
          <Rating rating={rating} showLabel={false} className="" />
        </div>
